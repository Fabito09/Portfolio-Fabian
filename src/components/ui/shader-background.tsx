...
/**
 * Ajustes tal como salieron del 21st.dev Shader Builder.
 * Se pueden sobrescribir por separado con la prop `uniforms`.
 */
export const SHADER_PRESET = {
  colors: [[0.043137254901960784,0.06274509803921569,0.14901960784313725],[0.23921568627450981,0.27450980392156865,0.9098039215686274],[0.6941176470588235,0.5490196078431373,1],[1,0.8392156862745098,0.9058823529411765],[1,0.8392156862745098,0.9058823529411765],[1,0.8392156862745098,0.9058823529411765],[1,0.8392156862745098,0.9058823529411765],[1,0.8392156862745098,0.9058823529411765]] as [number, number, number][],
  colorCount: 4,
  scale: 1.500,
  intensity: 0.550,
  paramA: 0.500,
  ...
  cursorEnabled: true,
  cursorEffect: 2.0,
  cursorStrength: 1.000,
  cursorRadius: 0.800,
  oklab: 0.0,
  timeScale: 0.176,
}

export type ShaderUniforms = typeof SHADER_PRESET
...
export function ShaderBackground({
  ...
  className?: string
  /** Sobrescribe solo los ajustes que se pasen; el resto sale del preset. */
  uniforms?: Partial<ShaderUniforms>
}) {
  const canvasRef = useRef<HTMLCanvasElement>(null)
  // El efecto solo debe reiniciarse si los valores cambian de verdad, no en
  ...
  useEffect(() => {
    const UNIFORMS: ShaderUniforms = {
      ...SHADER_PRESET,
      ...(JSON.parse(uniformsKey) as Partial<ShaderUniforms>),
    }
    ...
    // Arrow function (no `function`) para que TypeScript conserve dentro del
    // closure la garantía de que `canvas` y `gl` no son null.
